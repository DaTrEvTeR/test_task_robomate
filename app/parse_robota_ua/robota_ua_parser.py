...
class RobotaUaParser:
    def __init__(self, filter_obj: ResumeFilter, settings: Settings, session: CustomSession):
    ...
    async def get_resumes_page(self, request_data) -> dict[str, any]:
        ...
        """    
        async with self.session.post(self.settings.RESUMES_URL, json=request_data) as response:
            if not response.ok:
                raise ClientError(f"get_resumes_page: {response.status}: {response.reason}")
    ...
    async def get_page_count(self, request_data) -> int:
        ...
        async with self.session.post(self.settings.RESUMES_URL, json=request_data) as response:
            if not response.ok:
                raise ClientError(f"get_page_count: {response.status}: {response.reason}")
            res = await response.json()
        total = res["total"]
        if total:
            ...
            raise ValueError(f"get_page_count: Resumes not found")
        ...
        try:
            ...
            for page_num in range(page_count):
                ...
                for document in resumes_page["documents"]:
                    resumes_links.append(f"{self.settings.ONE_RESUME_URL}{document["resumeId"]}")

                results.append(self.get_resumes_data_from_urls(resumes_urls=resumes_links))
            res = []
    ...
    def get_resumes_data_from_urls(self, resumes_urls: list[str]) -> list[ResumeModel]:
        ...
        with Pool(processes=self.settings.processes_count) as pool:
            results = pool.map(process_wrapper, [(url_group, self.settings) for url_group in url_groups])
    ...
    def split_urls(self, urls: list[str]):
        ...
        """
        chunk_len = (len(urls) + self.settings.processes_count - 1) // self.settings.processes_count
        return (
...
async def process_urls(urls: list[str], settings: Settings) -> list[ResumeModel]:
    ...
    async with CustomSession() as session:
        tasks = [
            fetch_and_parse(url, session, settings)
            for url in urls
        ]
        return await asyncio.gather(*tasks, return_exceptions=True)

async def fetch_and_parse(url: str, session: CustomSession, settings: Settings) -> ResumeModel:
    """Fetch json with resume data and parse it"""
    async with asyncio.Semaphore(settings.process_max_connections):
        async with session.get(url) as response:
            if not response.ok:
                raise ClientError(f"fetch_and_parse: {response.status}: {response.reason}")
            res = await response.json()
